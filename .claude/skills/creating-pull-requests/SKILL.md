...
The evaluation-dashboard UI overstated listening-test progress: a 160-pair test would show `71 / 113  63%` instead of `71 / 160  44%`, and could flip to `complete` while a third of the manifest had never been annotated. This PR reads the true pair count from AnnotationHub's `download_sample_sources` API and patches `total_pairs` / `completion_pct` / `status` before they reach BigQuery.

**Files to review (4, +342 / -7):**

| File | Why |
|---|---|
| `ml-toolkit/listening_tests/sample_sources.py` *(new)* | Pure helpers. Start here. |
| `ml-toolkit/listening_tests/bq_export.py` | Calls AnnotationHub per tier; applies the override. |
| `tests/ml-toolkit_tests/test_lt_sample_sources.py` *(new)* | 10 unit tests for the helpers. |
