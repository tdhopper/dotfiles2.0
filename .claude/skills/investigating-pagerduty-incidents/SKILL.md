...
description: Generate weekly PagerDuty incident reports for audio-service and fusion services with root cause analysis from GCP logs. Outputs a Word document ready to paste into Slack.
---

You are a PagerDuty incident investigator for the Music Mission MIQ team at Spotify.

**Services**: audio-service, fusion
**Output**: Word document with actionable insights and debugging links

# Workflow

1. Fetch incidents from past week (resolved + open)
2. For audio-service: Query GCP logs at incident timestamps to find root causes
...
## Fetch Incidents
```bash
pd incident list --since "$(date -v-7d '+%Y-%m-%d')" --statuses resolved --services "audio-service" "fusion" --output json
pd incident list --services "audio-service" "fusion" --output json
```
...
## Query GCP Logs (audio-service only)
For each unique error pattern, query a 1-hour window around incident time:

```bash
# Find actual errors at incident timestamp
gcloud logging read 'resource.labels.container_name="audio-service-backend-worker" AND severity="ERROR" AND timestamp>="2025-11-19T22:00:00Z" AND timestamp<="2025-11-19T23:00:00Z"' \
  --project=audio-service-2 \
  --limit=30 \
  --format=json | \
...
gcloud logging read 'resource.labels.container_name="audio-service-backend-worker" AND severity="ERROR"' \
  --project=audio-service-2 \
  --limit=200 \
  --freshness=7d \
  --format=json | \
...
doc.add_paragraph('audio-service: 6 incidents, fusion: 1 incident', style='List Bullet')

# Key Issues (group by root cause)
doc.add_heading('Key Issues', level=2)

p = doc.add_paragraph()
p.add_run('Spotbot gRPC Timeouts (audio-service-backend-worker)').bold = True
...
add_hyperlink(p, 'https://oliver.spotify.net/ui/component/audio-service-backend-worker/incident', 'Oliver Investigation')
...
p = doc.add_paragraph()
add_hyperlink(p, 'https://console.cloud.google.com/logs/query;query=resource.labels.container_name%3D%22audio-service-backend-worker%22%0Aseverity%3D%22ERROR%22;duration=P7D?organizationId=642708779950&project=audio-service-2', 'GCP Logs - Backend Worker')
